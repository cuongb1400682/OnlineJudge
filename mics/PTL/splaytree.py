...
class SplayTreeNode(object):
    ...
    def rotate_left(self):
        ...
        if self.right is None:
            print("right hand is `None' when invoke rotate left on %s" % self, file=sys.stderr)
            return
        ...
        if self_right.left: self_right.left.parent = self
    ...
    def rotate_right(self):
        ...
        if self.left is None:
            print("left hand is `None' when invoke rotate right on %s" % self, file=sys.stderr)
            return
        ...
        if self_left.right: self_left.right.parent = self
        # connect x.left and x
        self_left.right = self
        self.parent = self_left


class SplayTreeIterator(object):
...
class SplayTree(object):
    ...
    def __iter__(self):
        return SplayTreeIterator(self.keys())
    ...
    def insert(self, key, val):
        ...
        if self._root is None:
            self._root = SplayTreeNode(key, val)
        else:
            ...
            if x is None:
                ...
                while node.left is not None:
                    node = node.left
                node.left = SplayTreeNode(key, val)
    ...
    def erase(self, key):
        ...
        y = find_max(left_tree)

        if y is None:
            y = find_min(right_tree)
            if y is None:
                self._root = None
                return
            self._splay(y)
